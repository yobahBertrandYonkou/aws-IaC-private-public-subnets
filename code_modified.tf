...
resource "aws_instance" "wordpress" {
  ...
  connection {
    ...
    private_key = file("C:\\Users\\YOBAH BERTRAND Y\\Desktop\\yob\\task3\\t3.pem")
  }
...
resource "aws_nat_gateway" "bmbnat"{
  depends_on = [aws_instance.wordpress]
  allocation_id = "eipalloc-0891c0b005dbbfe6e"
  subnet_id = aws_subnet.subnetpub.id
...
resource "aws_route_table" "natroutetb1"{
  depends_on = [aws_nat_gateway.bmbnat]
  vpc_id = aws_vpc.bmbvpc.id

  route{
    cidr_block = "0.0.0.0/0"
    nat_gateway_id = aws_nat_gateway.bmbnat.id
  }
